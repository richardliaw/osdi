  ...
  are then wrapped in Job and Task objects."""
    ...
    def make_job(self, role_name, num_tasks=1, **kwargs):
        ...
        if instances:
            ...
            print("Launching new job %s into VPC %s" % (job_name,
                                                        u.get_resource_name()))
            ...
            assert availability_zone in subnet_dict, "Availability zone %s is not in subnet dict, available subnets are %s" % (
                availability_zone, ', '.join(subnet_dict.keys()))
...
class Task(backend.Task):
    ...
    def __init__(self,
        ...
        self.scratch = "{}/{}.{}.{}.{}/scratch".format(
            TASKDIR_PREFIX, job._run.name, job.name, self.id,
            0)  # u.now_micros())
    ...
    def _initialize(self):
        ...
        while True:
            ...
            if self.ssh_client is None:
                self.log("SSH into %s:%s failed, retrying in %d seconds" %
                         (self.job.name, self.id, TIMEOUT_SEC))
                time.sleep(TIMEOUT_SEC)
            else:
                break
...
""".strip() % (self.keypair_fn, self.username, self.public_ip)
    ...
    def wait_until_ready(self):
        ...
        while not self._is_initialized_file_present():
            self.log(
                "wait_until_ready: Not initialized, retrying in %d seconds" %
                (TIMEOUT_SEC))
            time.sleep(TIMEOUT_SEC)
        ...
        if not self.cached_ip:
            ...
            for i in range(10):
                try:
                    ...
                    print(
                        "instance.load failed with %s, retrying in %d seconds"
                        % (str(e), retry_sec))
