...
class Job(backend.Job):
    def __init__(self, run, name, tmux_windows, install_script=None):
        ...
        for task_id, tmux_window in enumerate(tmux_windows):
            self.tasks.append(
                Task(
                    tmux_window, self, task_id, install_script=install_script))
...
class Task(backend.Task):
  ...
  from job name, and window names are task ids."""
    ...
    def __init__(self, tmux_window, job, task_id, install_script):
        ...
        print("Assigning %s:%s to port %s" % (self.job.name, self.id,
                                              self.port))
    ...
    def run(self, cmd, sync=True, ignore_errors=False):
        ...
        tmux_cmd = 'tmux send-keys -t {} {} Enter'.format(
            self.tmux_window, shlex.quote(modified_cmd))
        ...
        if contents != '0':
            if not ignore_errors:
                ...
                self.log("Warning: command %s returned status %s" % (cmd,
                                                                     contents))
