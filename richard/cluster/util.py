...
def get_keypair_name():
    ...
    validate_aws_name(
        username)  # if this fails, override USER with something nice
...
def get_placement_group_dict():
    ...
    for placement_group_response in response['PlacementGroups']:
        ...
        assert key not in result, ("Duplicate placement group " + key)
...
def get_available_zones():
    ...
    for avail_response in response['AvailabilityZones']:
        ...
        assert not messages, "zone %s is broken? Has messages %s" % (zone,
                                                                     message)
...
def create_efs(name):
    ...
    response = efs_client.create_file_system(
        CreationToken=token, PerformanceMode='generalPurpose')
    assert is_good_response(response)
    start_time = time.time()
    while True:
        try:
        ...
        if time.time() - start_time - WAIT_INTERVAL_SEC > WAIT_TIMEOUT_SEC:
            assert False, "Timeout exceeded creating EFS %s (%s)" % (token,
                                                                     name)
    ...
    response = efs_client.create_tags(
        FileSystemId=fs_id, Tags=u.make_name(name))
    assert is_good_response(response)
        ...
        while counter > 0:
            try:
                self.ssh_client.connect(
                    hostname=hostname, username=username, pkey=k)
                break
            except Exception as e:
                counter = counter - 1
                print(
...
def ssh_to_host(hostname, ssh_key=None, username=None, retry=1):
    """Create ssh connection to host
  ...
  """
    ...
    while counter > 0:
        try:
            ...
            counter = counter - 1
            print('Exception connecting to host via ssh (could be a timeout):'.
                  format(e))
            if counter == 0:
                return None
